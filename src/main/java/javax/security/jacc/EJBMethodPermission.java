...
/**
 * A security permission for ejb-method permissions. The name of an EJBMethodPermission contains the value of the ejb-name
 * element in the application's deployment descriptor that identifies the target EJB.
 * 
...
public final class EJBMethodPermission extends Permission implements Serializable {
     ...
     * 
     * 
     * A MethodInterfaceName is a non-empty String and should contain a method-intf value as defined for use in EJB deployment
     * descriptors. An implementation must be flexible such that it supports additional interface names especially if they are
     * standardized by the EJB Specification. The EJB Specification currently defines the following method-intf values:
     * 
     * { "Home", "LocalHome", "Remote", "Local", "ServiceEndpoint" }
     * 
     * 
     * A null or empty string methodSpec indicates that the permission applies to all methods of the EJB. A methodSpec with a
     * methodNameSpec of the empty string matches all methods of the EJB that match the methodInterface and methodParams
     * elements of the methodSpec.
     * 
     * A methodSpec with a methodInterfaceSpec of the empty string matches all methods of the EJB that match the methodNameSpec
     * and methodParamsSpec elements of the methodSpec.
     * 
     * A methodSpec without a methodParamsSpec matches all methods of the EJB that match the methodNameSpec and methodInterface
     * elements of the methodSpec.
     * 
     * The order of the typeNames in methodParams array must match the order of occurence of the corresponding parameters in the
     * method signature of the target method(s). Each typeName in the methodParams must contain the canonical form of the
     * corresponding parameter's typeName as defined by the getActions method. A methodSpec with an empty methodParamsSpec
     * matches all 0 argument methods of the EJB that match the methodNameSpec and methodInterfaceSpec elements of the
     * methodSpec.
     * 
    ...
    /**
     * Creates a new EJBMethodPermission with name corresponding to the EJBName and actions composed from methodInterface, and
     * the Method object.
     * 
     ...
     * @param methodInterface - A string that may be used to specify the EJB interface to which the permission pertains. A value
     *        of null or "", indicates that the permission pertains to all methods that match the other parameters of the
     ...
     * 
     * @param ejbName - the ejb-name of the target EJB
     * @param methodName - A string that may be used to indicate the method of the EJB to which the permission pertains. A value
     *        of null or "" indicates that the permission pertains to all methods that match the other parameters of the
    ...
    /**
     * Returns a String containing a canonical representation of the actions of this EJBMethodPermission. The Canonical form of
     * the actions of an EJBMethodPermission is described by the following syntax description.
     * 
     * methodNameSpec ::= methodName | emptyString methodInterfaceName ::= String methodInterfaceSpec ::= methodInterfaceName |
     * emptyString typeName ::= typeName | typeName [] methodParams ::= typeName | methodParams comma typeName methodParamsSpec
     * ::= emptyString | methodParams methodSpec ::= null | methodName | methodNameSpec comma methodInterfaceName |
     * methodNameSpec comma methodInterfaceSpec comma methodParamsSpec
     * 
     * 
     * The canonical form of each typeName must begin with the fully qualified Java name of the corresponding parameter's type.
     * The canonical form of a typeName for an array parameter is the fully qualified Java name of the array's component type
     * followed by as many instances of the string "[]" as there are dimensions to the array. No additional characters (e.g.
     * blanks) may occur in the canonical form.
     * 
     * A MethodInterfaceName is a non-empty String and should contain a method-intf value as defined for use in EJB deployment
     * descriptors. An implementation must be flexible such p it supports additional interface names especially if they are
     * standardized by the EJB Specification. The EJB Specification currently defines the following method-intf values: {
     * "Home", "LocalHome", "Remote", "Local", "ServiceEndpoint" }
     * 
    ...
    /**
     * Determines if the argument Permission is "implied by" this EJBMethodPermission. For this to be the case the following
     * must apply: The argument must be an instanceof EJBMethodPermission with name equivalent to p of this EJBMethodPermission,
     * and the methods to which the argument permission applies (as defined in its actions) must be a subset of the methods to
     * which this EJBMethodPermission applies (as defined in its actions).
     * 
     * The argument permission applies to a subset of the methods to which this permission applies if all of the following
     * conditions are met: - the method name component of the methodNameSpec of this permission is null, the empty string, or
     * equivalent to the method name of the argument permission - the method interface component of the methodNameSpec of this
     * permission is null, the empty string, or equivalent to the method interface of the argument permission - the method
     * parameter list component of the methodNameSpec of this permission is null, the empty string, or equivalent to the method
