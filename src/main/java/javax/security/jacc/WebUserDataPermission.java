 ...
 * 
 * The name of a WebUserDataPermission (also referred to as the target name) identifies a Web resource by its context path
...
public final class WebUserDataPermission extends Permission implements Serializable {
     ...
     *        contextPath (HttpServletRequest.getContextPath()). When the substring operation yields the string "/", the
     *        permission is constructed with the empty string as its name. The HTTP method component of the permission's actions
     *        is as obtained from HttpServletRequest.getMethod(). The TransportType component of the permission's actions is
     *        determined by calling HttpServletRequest.isSecure().
     */
    ...
    /**
     * Creates a new WebUserDataPermission with the specified name and actions. The name contains a URLPatternSpec that
     * identifies the web resources to which the permissions applies. The syntax of a URLPatternSpec is as follows:
     * 
     ...
     * 
     * The actions parameter contains a comma separated list of HTTP methods that may be followed by a transportType separated
     * from the HTTP method by a colon.
     * 
     ...
     * 
     * HTTPMethodSpec ::= emptyString | HTTPMethodExceptionList | HTTPMethodList
     * 
     * transportType ::= "INTEGRAL" | "CONFIDENTIAL" | "NONE"
     * 
     * actions ::= null | HTTPMethodSpec | HTTPMethodSpec colon transportType
     * 
     * 
     ...
     * 
     * @param name - the URLPatternSpec that identifies the application specific web resources to which the permission pertains.
     *        All URLPatterns in the URLPatternSpec are relative to the context path of the deployed web application module, and
     *        the same URLPattern must not occur more than once in a URLPatternSpec. A null URLPatternSpec is translated to the
     *        default URLPattern, "/", by the permission constructor.
     * @param actions - identifies the HTTP methods and transport type to which the permission pertains. If the value passed
     *        through this parameter is null or the empty string, then the permission is constructed with actions corresponding
     *        to all the possible HTTP methods and transportType "NONE".
     */
    ...
    /**
     * Creates a new WebUserDataPermission with name corresponding to the URLPatternSpec, and actions composed from the array of
     * HTTP methods and the transport type.
     * 
     * @param urlPatternSpec - the URLPatternSpec that identifies the application specific web resources to which the permission
     ...
     * @param httpMethods - an array of strings each element of which contains the value of an HTTP method. If the value passed
     *        through this parameter is null or is an array with no elements, then the permission is constructed with actions
     *        containing all the possible HTTP methods.
     * @param transportType - a String whose value is a transportType. If the value passed through this parameter is null, then
     *        the permission is constructed with actions containing transportType "NONE".
     */
     ...
     * @return true if the argument WebUserDataPermission object is equivalent to this, false otherwise.
     */
    public boolean equals(Object p) {
        ...
        /*
         * equals = urlSpec.equals(perm.urlSpec); if( equals == true ) { String a0 = getActions(); String a1 =
         * perm.getActions(); equals = (a0 != null && a0.equals(a1)) || (a0 == a1); } return equals;
         */
    }

    /**
     * Returns a canonical String representation of the actions of this WebUserDataPermission. The canonical form of the actions
     * of a WebUserDataPermission is described by the following syntax description. HTTPMethod ::= "Get" | "POST" | "PUT" |
     * "DELETE" | "HEAD" | "OPTIONS" | "TRACE" HTTPMethodList ::= HTTPMethod | HTTPMethodList comma HTTPMethod HTTPMethodSpec
     * ::= emptyString | HTTPMethodList transportType ::= "INTEGRAL" | "CONFIDENTIAL" | "NONE" actions ::= null | HTTPMethodList
     * | HTTPMethodSpec colon transportType
     * 
     * If the permission's HTTP methods include the entire HTTP method set and the permission's transport type is "INTEGRAL" or
     * "CONFIDENTIAL", the HTTP methods shall be represented in the canonical form by an empty string HTTPMethodSpec. If the
     * permission's HTTP methods include the entire HTTP method set and the permission's transport type is not "INTEGRAL"or
     * "CONFIDENTIAL", the canonical actions value shall be the null value. If the permission's methods do not include the
     * entire HTTP method set, duplicates must be eliminated and the remaining elements must be sorted into ascending lexical
     * order. The resulting HTTPMethodList must be included in the canonical form, and if the permission's transport type is not
     * "INTEGRAL" or "CONFIDENTIAL", the canonical actions value must be exactly the resulting HTTPMethodList.
     * 
     ...
     * 
     * - During the lifetime of a Java application, the hashCode method shall return the same integer value every time it is
     * called on a WebUserDataPermission object. The value returned by hashCode for a particular EJBMethod permission need not
     * remain consistent from one execution of an application to another. - If two WebUserDataPermission objects are equal
     ...
     * 
     * - The argument is an instanceof WebUserDataPermission. - The first URLPattern in the name of the argument permission is
     * matched by the first URLPattern in the name of this permission. - The first URLPattern in the name of the argument
     * permission is NOT matched by any URLPattern in the URLPatternList of the URLPatternSpec of this permission. - If the
     * first URLPattern in the name of the argument permission matches the first URLPattern in the URLPatternSpec of this
     * permission, then every URLPattern in the URLPatternList of the URLPatternSpec of this permission is matched by a
     * URLPattern in the URLPatternList of the argument permission. - The HTTP methods in the actions of the argument permission
     * are a subset of the HTTP methods in the actions of this permission. - The transportType in the actions of this permission
     * either corresponds to the value "NONE", or equals the transportType in the actions of the argument permission.
     * 
     * URLPattern matching is performed using the Servlet matching rules where two URL patterns match if they are related as
     * follows:
     * 
     * - their pattern values are String equivalent, or - this pattern is the path-prefix pattern "/*", or - this pattern is a
     * path-prefix pattern (that is, it starts with "/" and ends with "/*") and the argument pattern starts with the substring
